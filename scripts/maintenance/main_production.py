...
class OCCURCamSystem:
    ...
    def __init__(self, camera_source: str = "0", debug: bool = False):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            # Connect to camera
            if not self.camera_manager.connect_camera("MAIN_CAM", "usb", self.camera_source):
                logging.error(f"Failed to connect to camera {self.camera_source}")
                return False
    ...
    def _monitoring_loop(self):
        ...
        try:
            cap = cv2.VideoCapture(int(self.camera_source) if self.camera_source.isdigit() else 0)
    ...
    def _process_frame_for_auth(self, frame: np.ndarray, frame_count: int):
        ...
        try:
            ...
            recognized_faces = analysis.get_recognized_faces(0.6)
            unknown_faces = analysis.get_unknown_faces(0.6)
            
            # Handle recognized faces
            for recognition in recognized_faces:
                if recognition.employee_id and recognition.confidence > 0.6:
                    self._handle_known_user(recognition, frame, frame_count)
            
            # Handle unknown faces
            for detection in unknown_faces:
                self._handle_unknown_user(detection, frame, frame_count)
    ...
    def _handle_unknown_user(self, detection, frame: np.ndarray, frame_count: int):
        """Handle unknown user detection and registration."""
        try:
            # Extract face region
            face_region = self.face_engine.detector.extract_face_region(frame, detection)
            if face_region.size == 0:
                return
            
            # Generate unique user ID
            user_id = f"USER_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{frame_count}"
            ...
            # Register face with face engine
            face_photo_path = f"storage/reference_photos/{user_id}.jpg"
            os.makedirs(os.path.dirname(face_photo_path), exist_ok=True)
            
            # Save face image
            cv2.imwrite(face_photo_path, face_region)
            user_reg.face_photo_path = face_photo_path
            
            # Register with face engine
            success = self.face_engine.register_employee_face(
                user_id, face_region, face_photo_path
            )
            
            if success:
                # Save to database
                self._save_user_to_database(user_reg)
                
                # Add to known users
                self.known_users[user_id] = user_reg
                self.pending_registrations[user_id] = user_reg
                
                # Log registration
                self._log_authentication(user_id, "registered", detection.confidence, frame_count)
                
                self.stats["total_registrations"] += 1
                
                logging.info(f"Registered new user {user_id}")
            else:
                logging.warning(f"Failed to register user {user_id}")
                
        except Exception as e:
            logging.error(f"Error handling unknown user: {e}")
