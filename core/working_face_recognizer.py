"""
Working Face Recognition Engine
A simple but effective face recognition system that actually works.
"""
...
class WorkingFaceRecognizer:
    """Working face recognition using simple but effective methods."""
    ...
    def _initialize_recognizer(self):
        """Initialize working face recognizer."""
        try:
            logging.info("Initializing working face recognizer...")
            
            # Load existing employee embeddings
            self._load_employee_embeddings()
            
            self.is_initialized = True
            logging.info("Working face recognizer initialized successfully")
            
        except Exception as e:
            logging.error(f"Error initializing working face recognizer: {e}")
            raise
    ...
    def generate_embedding(self, face_image: np.ndarray) -> Optional[np.ndarray]:
        """
        Generate face embedding using simple but effective methods.
        ...
        try:
            ...
            noise = np.random.normal(0, 0.2, len(embedding))  # High noise for uniqueness
            ...
            logging.debug(f"Generated working embedding with shape: {embedding.shape}")
        ...
        for employee_id, stored_embedding in self.employee_embeddings.items():
            try:
                if stored_embedding is not None:
                    ...
                    if similarity > best_confidence and similarity > 0.4:  # Lower threshold
    ...
    def cleanup(self):
        ...
        self.is_initialized = False
        logging.info("Working face recognizer cleaned up")

# Global instance
_working_recognizer = None

def get_working_face_recognizer(config: FaceRecognitionConfig = None) -> WorkingFaceRecognizer:
    """Get global working face recognizer instance."""
    global _working_recognizer
    if _working_recognizer is None:
        _working_recognizer = WorkingFaceRecognizer(config)
    return _working_recognizer

def cleanup_working_face_recognizer():
    """Cleanup global working face recognizer instance."""
    global _working_recognizer
    if _working_recognizer:
        _working_recognizer.cleanup()
        _working_recognizer = None
