"""
Proper Face Recognition System
Based on multi-cam-face-tracker repository approach.
"""
...
class ProperFaceRecognizer:
    """Proper face recognition system based on proven methods."""
    
    def __init__(self, config: FaceRecognitionConfig = None):
        """Initialize proper face recognizer."""
    ...
    def _initialize_recognizer(self):
        """Initialize proper face recognizer."""
        try:
            logging.info("Initializing proper face recognizer...")
            
            # Load existing employee embeddings
            self._load_employee_embeddings()
            
            self.is_initialized = True
            logging.info("Proper face recognizer initialized successfully")
            
        except Exception as e:
            logging.error(f"Error initializing proper face recognizer: {e}")
            raise
    ...
    def generate_embedding(self, face_image: np.ndarray) -> Optional[np.ndarray]:
        """
        Generate face embedding using proper feature extraction.
        ...
        try:
            ...
            # Add unique noise based on image content for uniqueness
            ...
            logging.debug(f"Generated proper embedding with shape: {embedding.shape}")
    ...
    def cleanup(self):
        ...
        self.is_initialized = False
        logging.info("Proper face recognizer cleaned up")

# Global instance
_proper_recognizer = None

def get_proper_face_recognizer(config: FaceRecognitionConfig = None) -> ProperFaceRecognizer:
    """Get global proper face recognizer instance."""
    global _proper_recognizer
    if _proper_recognizer is None:
        _proper_recognizer = ProperFaceRecognizer(config)
    return _proper_recognizer

def cleanup_proper_face_recognizer():
    """Cleanup global proper face recognizer instance."""
    global _proper_recognizer
    if _proper_recognizer:
        _proper_recognizer.cleanup()
        _proper_recognizer = None
